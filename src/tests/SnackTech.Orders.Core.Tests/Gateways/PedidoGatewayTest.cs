...
public class PedidoGatewayTest
{
    ...
    public PedidoGatewayTest()
    {
        ...
        _pedidoExemplo = new Pedido(
            ...
            new List<PedidoItem>
            {
                new PedidoItem(
                    Guid.NewGuid(),
                    new Produto(Guid.NewGuid(), 10.99m),
                    2,
                    "observacao"
                )
            }
    ...
    public void ConverterParaEntidade_DeveRetornarPedido_ComDadosCorretos()
    {
        ...
        pedido.Itens[0].Produto.Id.Valor.Should().Be(_pedidoDtoExemplo.Itens.First().ProdutoId);
        pedido.Itens[0].Produto.Valor.Valor.Should().Be(_pedidoDtoExemplo.Itens.First().ValorUnitario);
        pedido.Itens[0].Valor().Valor.Should().Be(_pedidoDtoExemplo.Itens.First().ValorTotal);
    }
    ...
    public void ConverterParaDto_DeveRetornarPedidoDto_ComDadosCorretos()
    {
        ...
        pedidoDto.Itens.First().ProdutoId.Should().Be(_pedidoExemplo.Itens.First().Produto.Id.Valor);
        pedidoDto.Itens.First().ValorUnitario.Should().Be(_pedidoExemplo.Itens.First().Produto.Valor);
        pedidoDto.Itens.First().ValorTotal.Should().Be(_pedidoExemplo.Itens.First().Valor());
    }
